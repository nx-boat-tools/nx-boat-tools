...
import * as localChartGenerator from '../test/generator';
...
describe('test-project generator', () => {
  ...
  each([1, 2]).describe('workspace v%s', (workspaceVersion) => {
    ...
    it('successfully calls the dotnet test Generator', async () => {
      const options: DotnetTestProjectGeneratorSchema = {
        name: 'my-project',
        testPrefix: 'integration',
        testType: 'xunit',
      ...
      expect(schemaArg.testPrefix).toBe(options.testPrefix);
      expect(schemaArg.testType).toBe(options.testType);
