...
describe('common chain-stage generator', () => {
  let appTree: Tree;
  ...
  it('fails when project does not exist', async () => {
    const options: CommonChainStageGeneratorSchema = {
      project: 'test',
  ...
  it('creates chain-execute target when target does not already exists', async () => {
    ...
    expect(config.targets.chainTarget.options?.stages?.src?.preTargets[0]).toBe(
      'pre1'
    );
    expect(config.targets.chainTarget.options?.stages?.src?.preTargets[1]).toBe(
      'pre2'
    );
  ...
  it('adds to chain-execute target when target already exists (existing build chain-execute with stage)', async () => {
    ...
    expect(config.targets.chainTarget.options?.stages?.src?.preTargets[2]).toBe(
      'pre2'
    );
