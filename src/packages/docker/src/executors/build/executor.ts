...
export default async function runExecutor(options: BuildExecutorSchema, context: ExecutorContext) {
  ...
  if (!existsSync(dockerFilePath)) {
      throw new Error('A dockerfile could not be found in the specified build directory.');
  }

  console.log(`\n🔨 Building docker image '${projectName}'...\n`);
  console.log(await spawnAsync(`docker build -t ${projectName}:latest -f ${dockerFilePath} ${buildPath}`));
  ...
  if(version !== undefined) {
    ...
    console.log(await spawnAsync(`docker tag ${projectName}:latest ${projectName}:${version}`));
  }
