...
export function getAllProjectsFromSolution(slnContent: string, basePath: string): Array<string> {
...
export function appendProjectLinesToSolution(rootSolutionContents: string, projectSolutionContents: string): string {
  ...
  export function appendGlobalSectionToSolution(rootSolutionContents: string, projectSolutionContents: string): string {
    const globalSectionRegex = /([\s\S]*GlobalSection\(ProjectConfigurationPlatforms\) = postSolution)([\s\S]*)(\r\n\tEndGlobalSection[\s\S]*)/;
